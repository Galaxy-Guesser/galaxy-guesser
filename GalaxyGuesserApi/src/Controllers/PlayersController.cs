  ...
  public class PlayersController : ControllerBase
  {
    ...
    public async Task<ActionResult<ApiResponse<IEnumerable<PlayerStatsDTO>>>> GetPlayersStats(int playerId)
    {
      try
      {
        ...
        var stats = await _playerService.GetPlayersStats(playerId);

        if (stats == null || !stats.Any())
        {
          return NotFound(ApiResponse<IEnumerable<PlayerStatsDTO>>.ErrorResponse($"No stats found for player with ID {playerId}"));
        }

        return Ok(ApiResponse<IEnumerable<PlayerStatsDTO>>.SuccessResponse(stats, "Player stats retrieved successfully"));
      }
      catch (Exception ex)
      {
    ...
    [ProducesResponseType(typeof(ApiResponse<Player>), StatusCodes.Status404NotFound)]
    [ProducesResponseType(typeof(ApiResponse<Player>), StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<ApiResponse<Player>>> UpdatePlayerUsernameAsync([Required] int playerId, [FromBody] UpdatePlayerRequest request)
    {
      try
      {
        var unauthorizedResult = ValidateAuthentication<Player>();
