...
namespace GalaxyGuesserCLI.Services
{
    public class SessionViewService
    {
        ...
        private const string ActiveSessionsApiUrl = "http://ec2-13-244-67-213.af-south-1.compute.amazonaws.com/api/sessions"; 

        private const string AllSessionsApiUrl = "http://ec2-13-244-67-213.af-south-1.compute.amazonaws.com/api/sessions/all";
        ...
        public static async Task<List<SessionView>> GetAllSessions()
        {
            try
            {
                ...
                HttpResponseMessage response = await _httpClient.GetAsync(AllSessionsApiUrl);
                response.EnsureSuccessStatusCode();
